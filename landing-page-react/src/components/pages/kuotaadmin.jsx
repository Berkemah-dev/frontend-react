...
const Kuotaadmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editMode) {
      try {
        const response = await axios.put(`http://localhost:5000/api/kuota/updated-kuota/${editId}`, formData);
      ...
      try {
        const response = await axios.post('http://localhost:5000/api/kuota/create_kuota', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/kuota/delete/${id}`);
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <label htmlFor="destinasi" className="block text-sm font-medium text-gray-700">Destinasi</label>
            <input
              type="text"
              name="destinasi"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            <label htmlFor="kuota" className="block text-sm font-medium text-gray-700">Kuota</label>
            <input
              type="number"
              name="kuota"
          ...
          <div>
            <label htmlFor="sisa_kuota" className="block text-sm font-medium text-gray-700">Sisa Kuota</label>
            <input
              type="number"
              name="sisa_kuota"
          ...
          <li key={kuota._id} className="bg-white p-4 rounded-lg shadow-md">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-600">Kuota: {kuota.kuota}, Sisa Kuota: {kuota.sisa_kuota}</p>
              </div>
